...
type CartContextType = {
  ...
  setCart: React.Dispatch<React.SetStateAction<CartItem[]>>; // ✅ Added setCart
};
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const clearCart = () => {
    setCart([]);
    localStorage.removeItem('cart'); // Clear cart from local storage when cleared
  };
  ...
  return (
    <CartContext.Provider value={{ cart, wishlist, addToCart, removeFromCart, updateCartQuantity, clearCart, addToWishlist, removeFromWishlist, setCart }}>
      {children}
    </CartContext.Provider>
  );
};
